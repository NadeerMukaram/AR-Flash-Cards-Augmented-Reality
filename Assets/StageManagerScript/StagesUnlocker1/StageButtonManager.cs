...
public class StageButtonManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        stageButtonUnlocker = FindObjectOfType<StageButtonUnlocker>();
    }
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < stageButtons.Length && i < stageButtonUnlocker.stageIsUnlocked.Length; i++)
        {
            int stageNumber = i + 2; // Adjust for stages starting from 2
            if (stageButtonUnlocker.stageIsUnlocked[i])
            {
